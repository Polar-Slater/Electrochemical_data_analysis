...
## DRT_tools improved notes

This local improved copy keeps the original pyDRTtools workflow, but adds a more forgiving EIS importer for CHI files.

- CHI-extracted EIS tables with headers such as `Freq/Hz`, `Z'/ohm`, and `Z"/ohm` now import directly.
- Raw CHI text exports are scanned for the EIS data table after the metadata block.
- Headerless 3-column `.txt` and `.csv` files from the original pyDRTtools still work.
- Imported frequencies are cleaned, filtered to positive values, and sorted from high to low frequency.
- GUI import failures now show a clear error dialog instead of failing silently.
- Package import is quieter and lighter, so helper modules can be used without loading the full GUI/numerics stack.
