...
class EIS_object(object):
    ...
    def __init__(self, freq, Z_prime, Z_double_prime):
        ...
        Z_double_prime = np.asarray(Z_double_prime, dtype=float)
        if not (freq.shape == Z_prime.shape == Z_double_prime.shape):
            raise ValueError("freq, Z_prime, and Z_double_prime must have the same shape.")
        if freq.ndim != 1 or freq.size < 3:
            raise ValueError("EIS data must contain at least 3 one-dimensional points.")
        if np.any(freq <= 0):
    ...
    def from_file(cls,filename):
        freq, Z_prime, Z_double_prime = load_eis_file(filename)
