...
#
from .runs import *
from .eis_io import EISFileError
#
...
class GUI(QtWidgets.QMainWindow):
    ...
    def import_file(self):
        ...
        path, ext = QFileDialog.getOpenFileName(None, "Please choose a file", "", "EIS files (*.csv *.txt *.dat);;All Files (*)") 
        ...
        except (EISFileError, ValueError, IndexError) as exc:
            QtWidgets.QMessageBox.critical(self, "Could not import EIS file", str(exc))
            return
    ...
    def inductance_callback(self): 
        ...
        if self.data is None:
            ...
            capacitive_mask = -self.data.Z_double_prime > 0
            if np.count_nonzero(capacitive_mask) < 3:
                QtWidgets.QMessageBox.warning(
                    self,
                    "Not enough capacitive points",
                    "Discarding inductive data would leave fewer than 3 points, so the full dataset is kept.",
                )
                signals_blocked = self.ui.induct_choice.blockSignals(True)
                self.ui.induct_choice.setCurrentIndex(0)
                self.ui.induct_choice.blockSignals(signals_blocked)
                ...
                self.data.freq = self.data.freq[capacitive_mask]
                self.data.Z_prime = self.data.Z_prime[capacitive_mask]
                self.data.Z_double_prime = self.data.Z_double_prime[capacitive_mask]
...
class Figure_Canvas(FigureCanvas):
    ...
    def EIS_data(self, entry): # plot the data
        ...
        if entry.method == 'BHT': # for BHT run
            self.axes.plot(entry.mu_Z_re, -entry.mu_Z_im,
                           'k', label = r'$Z_\mu$(Regressed)', linewidth=3)
        ...
        self.axes.set_xlabel(r'$Z^{\prime}/\Omega$')
        self.axes.set_ylabel(r'-$Z^{\prime \prime}/\Omega$')
    ...
    def Magnitude(self, entry): # plot the magnitude
        ...
        if entry.method == 'BHT': # for BHT run
            self.axes.semilogx(entry.freq, absolute(entry.mu_Z_re+1j*entry.mu_Z_im), 
                               'k', label = r'$Z_\mu$(Regressed)', linewidth=2)
        ...
        self.axes.set_ylabel(r'$|Z|/\Omega$')
    ...
    def Re_data(self, entry): # plot the real part
        ...
        if entry.method == 'BHT': # for BHT run
            ...
            self.axes.semilogx(entry.freq, entry.mu_Z_re, 'k', label = r'$Z_\mu$(Regressed)', linewidth=3)
        ...
        self.axes.set_xlim([min(entry.freq), max(entry.freq)])    
        self.axes.set_xlabel('$f/Hz$')
        self.axes.set_ylabel(r'$Z^{\prime}/\Omega$')
    ...
    def Im_data(self, entry): # plot of the imaginary part
        ...
        if entry.method == 'BHT': # for BHT run
            ...
            self.axes.semilogx(entry.freq, -entry.mu_Z_im, 'k', label = r'$Z_\mu$(Regressed)', linewidth=3)
    ...
    def Re_residual(self, entry): # plot the residuals of the real part of the impedance
        ...
        if entry.method == 'none' : # or str(entry.ui.data_used_choice.currentText()) == 'Im Data':
            ...
            self.axes.set_xlabel('$f/Hz$')
            self.axes.set_ylabel(r'$R_{\infty}+Z^{\prime}_{H}-Z^{\prime}_{exp}/\Omega$')
            y_max = max(3*entry.band_re_agm)
        
        else: # for simple or Bayesian run
            self.axes.semilogx(entry.freq, entry.res_re, 'or')
            self.axes.set_xlabel('$f/Hz$')
            self.axes.set_ylabel(r'$Z^{\prime}_{DRT}-Z^{\prime}_{exp}/\Omega$')
    ...
    def Im_residual(self, entry): # plot the residuals of the imaginary part of the impedance
        ...
        if entry.method == 'none' : # or str(entry.ui.data_used_choice.currentText()) == 'Re Data':
            ...
            self.axes.set_xlabel('$f/Hz$')
            self.axes.set_ylabel(r'$\omega L_0+Z^{\prime \prime}_{H}-Z^{\prime\prime}_{exp}/\Omega$')
            y_max = max(3*entry.band_im_agm)
            
        else: # for simple or Bayesian run
            self.axes.semilogx(entry.freq, entry.res_im, 'or')
            self.axes.set_xlabel('$f/Hz$')
            self.axes.set_ylabel(r'$Z^{\prime \prime}_{DRT}-Z^{\prime \prime}_{exp}/\Omega$')
